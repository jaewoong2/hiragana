  ...
  return (
    <div className="App">
      ...
      <div className="character-display">
        ...
        {showAnswer && (
          <button type="button" className="answer" onClick={onClickNextButton}>
            다음
          </button>
        )}
      </div>
      <div className={`options ${shake ? 'shake-animation' : ''}`}>
        {options.map((option) => (
          <button key={option} onClick={() => checkAnswer(option)} type="button">
            {option}
          </button>
        ))}
      </div>
      ...
      <div className="wrong-answers">
        ...
        <ul>
          {wrongAnswers.map((item) => (
            <li key={item.character} title={`정답은 ${item.romanization} 입니다.`}>
